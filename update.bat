...
powershell -command Invoke-WebRequest -Uri http://thegearmc.com/update/btversion.txt -OutFile tasks/version_dummy.txt

@echo Current Version
findstr /c:"%v%" /i tasks\version.txt
set RESULT1=%ERRORLEVEL%
echo.
echo.
@echo Current Update Version.
findstr /c:"%v%" /i tasks\version_dummy.txt
set RESULT2=%ERRORLEVEL%
set v2=
for /f "delims=" %%i in ('type tasks\version_dummy.txt') do set v2=%%i
echo.
cls
if %RESULT1%==%RESULT2% (
    ...
    powershell -command Invoke-WebRequest -Uri http://thegearmc.com/update/btversion.txt -OutFile tasks/version.txt
)
:exit
echo.
@pause
